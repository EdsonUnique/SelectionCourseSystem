	...
	<body>
		...
		<div class="search">
			...
			<table cellpadding="10px" cellspacing="2px">
				<tr>
					<td>
						<input id="tea_id" type="text" name="tea_id" style="border-radius: 20px;text-indent: 20px;height: 25px;">
					</td>
					<td>
						<button onclick="javascript: checkTeaId();">按工号查询</button>
					</td>
					<td>
						<input id="tea_name" type="text" name="tea_name" style="border-radius: 20px;text-indent: 20px;height: 25px;">
					</td>
					<td>
						<button onclick="javascript: checkTeaName();">按姓名查询</button>
					</td>
				</tr>
			</table>
			</div>
		...
		<div class="display_all">
			...
			<table cellpadding="10px" cellspacing="2px" border="1px solid" style="text-align: center;">
				...
				<s:iterator value="%{#session.pb.pageData}" var="tea">
				<tr>
					<td>
						<s:property value="%{#tea.tea_id}"/>
					</td>
					<td>
						<s:property value="%{#tea.tea_name}"/>
					</td>
					<td>
						<s:if test="%{#tea.tea_gender==0}">女</s:if>
						<s:else>男</s:else>
					</td>
					<td>
						<s:property value="%{#tea.tea_pwd}"/>
					</td>
					<td>
						<s:property value="%{#tea.tea_birth}"/>
					</td>
					<td>
						<button onclick="javascript:window.location.href='${pageContext.request.contextPath}/adminTea_updateTeacherUI?tea_id='+'${tea.tea_id}'">修改</button>
					</td>
					<td>
						<button onclick="javascript:deleteTea(${tea.tea_id});">删除</button>
					</td>
				</tr>
				</s:iterator>
			</table>
